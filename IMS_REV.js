...
function addItem() {
    ...
    reader.onload = function () {
        ...
        if (editingRow) {
            editingRow.cells[0].innerHTML = `<img src="${imageSrc}" class="item-img">`;
            editingRow.cells[1].innerText = name;
            editingRow.cells[2].innerText = details;
            editingRow.cells[3].innerText = quantity;
            editingRow.cells[4].innerText = price;
            editingRow.cells[5].innerText = type;
            editingRow = null;
        } else {
            let row = table.insertRow();
            row.innerHTML = `
                <td><img src="${imageSrc}" class="item-img"></td>
                <td>${name}</td>
                <td>${details}</td>
                <td>${quantity}</td>
                <td>${price}</td>
    ...
    if (imageInput.files.length > 0) {
        ...
        reader.onload(); // Fallback to placeholder or no image
    }
}
